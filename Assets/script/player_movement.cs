...
//        main_char.GetCurrentAnimatorStateInfo(0).IsName("Attack2")
        if (punch==0) { 
        movement = new Vector3(x_input * walkspeed, 0.0f, 0.0f);
    ...
    void Movement_controller() {
        ...
        if(isJumping == true && temp_gap_y < 0.0f && transform.position.y <= axisY + 1.3f && transform.position.y > axisY + 0.05f)
        {
